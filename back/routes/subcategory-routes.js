...
router.post(
  ...
  [
    ...
    check(
      "id_category",
      "El id_categoria debe tener al menos un valor"
    ).isLength({ min: 1 }),
    check("id_category")
      .custom(categoryExist)
      .withMessage("La categoria asignada no existe en el sistema."),
    validarCampos,
  ],
  validateJWT,
  isRolValid(abilities.createSubcategory),
...
router.delete(
  "/delete/:id",
  validateJWT,
  isRolValid(abilities.deleteSubcategory),
...
router.put(
  "/update",
  validateJWT,
  isRolValid(abilities.updateSubcategory),
